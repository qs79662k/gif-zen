 ...
 * 帧数据缓存的默认驱逐策略工厂。
 * <p>
 * 该工厂根据给定的上下文动态生成一个 {@link EvictionPolicy}，用于决定在原始帧数据缓存
 * 空间不足时哪些帧可以被驱逐。其核心保留规则基于双缓存窗口（Primary / Wraparound）机制，
 * 旨在最大化缓存命中率，减少重复的 I/O 读取操作。
 * </p>
 *
 * <p><b>保留规则：</b>
 * <ol>
 *   <li>另一路径的所有帧（若当前为主路径，则保留回绕路径的所有帧；反之亦然）</li>
 *   <li>当前帧索引 +2（即下一个预取读取的帧索引）</li>
 *   <li>正向播放时，另一路径边界之后的连续两帧（用于预加载连续性）</li>
 * </ol>
 * 不在保留集内的帧将被视为可驱逐。
 * </p>
 *
 ...
 * <p><b>典型用法：</b>
 * <pre>{@code
 * EvictionPolicyFactory<Integer> factory = new DefaultFrameDataEvictionPolicyFactory();
 ...
 *
 * // 后续与缓存管理器结合使用
 * frameDataCache.evictEntries(policy);
 * }</pre>
 *
 * @author wsp
 * @version 1.0
 ...
 */
public class DefaultFrameDataEvictionPolicyFactory implements EvictionPolicyFactory<Integer> {
    ...
    public EvictionPolicy<Integer> create(EvictionPolicyContext context) {
        ...
        // 预取读取索引：当前帧的下两帧（循环取模）
        int readIdx = (currentFrameIndex + 2) % totalFrames;
        ...
        // 正向播放时，计算另一路径边界之后的连续两帧索引（用于预加载连续性）
        int next1 = -1, next2 = -1;
        if (isForwardDirection && otherRange != null && !otherRange.isEmpty()) {
            next1 = (otherRange.startIndex + otherRange.frameCount) % totalFrames;
            next2 = (next1 + 1) % totalFrames;
        }

        // 构建最终保留集：另一路径全部帧 + 预取读取帧 + 边界后续两帧
        Set<Integer> readKeep = new HashSet<>(otherBase);
        readKeep.add(readIdx);
        if (next1 != -1) readKeep.add(next1);
        if (next2 != -1) readKeep.add(next2);

        return new DefaultEvictionPolicy<>(readKeep);
    }
}