...
/**
 * {@code byte[]} 的默认对象工厂实现，支持动态学习数组长度（线程安全）。
 * <p>
 * 该工厂不预设数组长度，而是在首次归还对象时通过 {@link #reset(byte[])} 学习实际长度，
 * 后续 {@link #create()} 将按学习到的最新长度生成字节数组。
 * </p>
 *
 * <p><b>长度学习策略：</b>
 ...
 *   <li>调用 {@link #reset(byte[])} 时，工厂会从归还的数组中提取长度并更新内部记录（线程安全）。</li>
 ...
 *
 * <p><b>线程安全性：</b>
 * 使用 {@code volatile} 保证可见性。{@link #reset(byte[])} 与 {@link #clear()}
 ...
 * @see ObjectFactory
 */
public class DefaultByteArrayFactory implements ObjectFactory<byte[]> {
     ...
     * </p>
     *
     * @return 新创建的字节数组，若长度尚未初始化则返回 {@code null}
     */
    @Override
    public byte[] create() {
     ...
     * </p>
     *
     * @param obj 归还的 {@code byte[]} 实例，可以为 {@code null}（若为 {@code null} 则直接返回）
     */
    @Override
    public synchronized void reset(byte[] obj) {
        if (obj == null) {
            return;
        }
     ...
     * 调用后，{@link #create()} 将再次返回 {@code null}，直到通过 {@link #reset(byte[])}
     * 重新学习新长度。
     * </p>
     * <p>
     * <b>线程安全性：</b> 该方法使用 {@code synchronized} 修饰，与 {@link #reset(byte[])}
     * 互斥执行，避免并发重置导致状态不一致。
     * </p>
     * <p>
     ...
     * </p>
     *
     * @param obj 需要验证的字节数组，由池保证不为 {@code null}
